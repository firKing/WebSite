...
-- --------------------------------------------------
-- Date Created: 01/27/2015 18:22:04
...
ALTER TABLE [dbo].[companies]
ADD CONSTRAINT [FK_usercompany1]
    FOREIGN KEY ([userId])
...
-- Creating non-clustered index for FOREIGN KEY 'FK_usercompany1'
CREATE INDEX [IX_FK_usercompany1]
ON [dbo].[companies]
    ([userId]);
GO

-- Creating foreign key on [userId] in table 'experts'
ALTER TABLE [dbo].[experts]
ADD CONSTRAINT [FK_userexpert1]
    FOREIGN KEY ([userId])
    REFERENCES [dbo].[users]
        ([userId])
    ON DELETE NO ACTION ON UPDATE NO ACTION;
...
-- Creating non-clustered index for FOREIGN KEY 'FK_userexpert1'
CREATE INDEX [IX_FK_userexpert1]
ON [dbo].[experts]
    ([userId]);
GO
...
-- Creating non-clustered index for FOREIGN KEY 'FK_uservendor1'
CREATE INDEX [IX_FK_uservendor1]
ON [dbo].[vendors]
    ([userId]);
GO
